...
const itemCartDOM = get('.cart-items');

const addToCartDom = (newCart) => {
  const { id, name, price, img, amount } = newCart;
  const article = document.createElement('article');
  article.classList.add('cart-item');
  article.setAttribute('data-id', id);
  article.innerHTML = ` <img src="${img}" alt="${name}" class="cart-item-img">
            <div>
  ...
  itemCartDOM.appendChild(article);
};
